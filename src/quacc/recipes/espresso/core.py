...
def d3hess_job(
    ...
    **calc_kwargs,
    ...
    """
    Function to carry out a basic d3hess.x calculation. It should allow you to
    use all the features of the [d3hess.x binary](https://www.quantum-espresso.org/Doc/INPUT_D3HESS.html)

    `d3hess.x` is a post processing program to compute second derivatives of D3 dispersion.
    It should be run after an scf or relax calculation. It reads the molecular geometry from
    the xml file in the outdir and saves the second derivatives matrix in a file on the disk.
    
    Phonon reads the file and add the D3 Hessian matrix to the dynamical matrix with the proper phase (q),
